...
def create_dataloader(dataset, test=False):
    if not test:
        dataloader = torch.utils.data.DataLoader(dataset, num_workers=5, batch_size=4, shuffle=True)
    else:
        dataloader = torch.utils.data.DataLoader(dataset, num_workers=5, batch_size=1, shuffle=False)
    return dataloader
