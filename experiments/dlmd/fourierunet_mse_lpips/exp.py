...
lpips_weight = 0.0
lpips_step_size=0.1
lpips_step_milestones=[23000, 46000, 92000, 115000, 115001]
...
downsample = 4
downsampled_image_shape = tuple(int(s / downsample) for s in image_shape)

def create_reconstruction_network():
    # create multi gpu reconstruction network list for 1 gpu
    deconv = d.FourierUNetRGB(downsampled_image_shape,
                                       scale_factors=[1, 2, 4, 8],
                                       funet_fmaps=[64, 64, 64, 64],
                                       conv_kernel_size=(1, 11, 11),
                                       conv_padding=(0, 5, 5),
...
def initialize_optimizer(deconv, latest=None):
    ...
    opt = optim.Adam([{'params': deconv.parameters(), 'lr': learning_rate}],
                     lr=learning_rate)
    if latest is not None:
...
def create_dataloader(dataset, test=False):
    if not test:
        dataloader = torch.utils.data.DataLoader(dataset, num_workers=4, batch_size=4, shuffle=True)
    else:
        dataloader = torch.utils.data.DataLoader(dataset, num_workers=4, batch_size=1, shuffle=False)
    return dataloader
...
def train():
    ...
    if latest is not None:
        ...
        validate_mses = latest['validate_high_pass_mses']
    else:
        latest_iter = 0
        mses = []
        lpips_losses = []
    ...
    # run psf training
    control.train_rgb_recon(deconv,
                            opt,
                            dataloader,
                            devices,
                            mses,
...
def test():
    # initialize model for training
    if os.path.exists('latest.pt'):
        latest = torch.load('snapshots/state230000.pt')
    else:
        latest = None
    deconv = initialize_reconstruction(latest=latest)
    deconv.eval()
