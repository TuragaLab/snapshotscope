...
slm_k = (np.linspace(-slm_num_pixels // 2 + 1,
                     slm_num_pixels // 2 + 1,
                     num=slm_num_pixels,
                     endpoint=True,
                     dtype=np.float32)
...
augmentations = augment.create_augmentations([{'name': 'adjust_brightness',
                                               ...
                                               {'name': 'slice_crop',
                                                'args': {'num_planes': int(subsampled_num_planes / 10)}},
                                               {'name': 'pad_volume',
...
def create_reconstruction_networks():
    ...
    deconvs = [d.FourierNet3D(int(640 / num_scopes),
                                                 (downsampled_num_pixels, downsampled_num_pixels),
                                                 int(subsampled_num_planes / num_scopes),
                                                 fourier_conv_args={'stride': 2},
                                                 conv_kernel_sizes=[(11, 7, 7), (11, 7, 7)],
                                                 conv_fmap_nums=[int(int(640 / num_scopes) / int(subsampled_num_planes / num_scopes)), 1],
                                                 imbn_momentum=0.1,
...
def initialize_microscope_reconstruction(latest=None, phase_mask_init=None):
    ...
    pixels = create_phase_mask(mics[0].kx,
                               mics[0].ky,
...
def initialize_optimizer(micdeconv, latest=None):
    ...
    opt = lfmoptim.RAdam([{'params': micdeconv['deconvs'].parameters(), 'lr': learning_rate}],
                         lr=learning_rate)
    if latest is not None:
...
def create_test_extents():
    ...
    extents = [[((0, subsampled_num_planes), (280, 280 + length), (120, 120 + length)), ((0, subsampled_num_planes), (600, 600 + length), (138, 138 + length))],
               [((0, subsampled_num_planes), (280, 280 + length), (138, 138 + length)), ((0, subsampled_num_planes), (550, 550 + length), (138, 138 + length))],
               [((0, subsampled_num_planes), (190, 190 + length), (138, 138 + length)), ((0, subsampled_num_planes), (400, 400 + length), (138, 138 + length))],
               [((0, subsampled_num_planes), (200, 200 + length), (138, 138 + length)), ((0, subsampled_num_planes), (500, 500 + length), (150, 150 + length))],
               [((0, subsampled_num_planes), (155, 155 + length), (120, 120 + length)), ((0, subsampled_num_planes), (400, 400 + length), (150, 150 + length))],
               [((0, subsampled_num_planes), (190, 190 + length), (138, 138 + length)), ((0, subsampled_num_planes), (460, 460 + length), (138, 138 + length))],
               [((0, subsampled_num_planes), (230, 230 + length), (138, 138 + length)), ((0, subsampled_num_planes), (520, 520 + length), (138, 138 + length))],
               [((0, subsampled_num_planes), (230, 230 + length), (138, 138 + length)), ((0, subsampled_num_planes), (520, 520 + length), (138, 138 + length))],
               [((0, subsampled_num_planes), (190, 190 + length), (138, 138 + length)), ((0, subsampled_num_planes), (460, 460 + length), (138, 138 + length))],
               [((0, subsampled_num_planes), (240, 240 + length), (120, 120 + length)), ((0, subsampled_num_planes), (460, 460 + length), (138, 138 + length))],]
    return extents
...
def train():
    ...
    phase_mask_init = torch.load('../psf_optimization/snapshots/phase_mask580000.pt',
