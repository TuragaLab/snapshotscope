...
def initialize_optimizer(micdeconv, latest=None):
    ...
    opt = optim.Adam([{'params': micdeconv['placeholder_deconvs'].parameters(), 'lr': learning_rate},
...
def create_dataloader(test=False):
    ...
    if not test:
        dataset = data.ConfocalVolumesDataset([os.path.join(base_path, "zjabc_train/2019-12-10-6dpf-Huc-H2B-jRGECO"),
                                              ...
                                              shape=(subsampled_num_planes, downsampled_num_pixels, downsampled_num_pixels),
                                              location='random',
                                              strategy='center',
                                              bounds_roi='data',
        ...
        dataset = data.ConfocalVolumesDataset([os.path.join(base_path, "zjabc_test/2019-02-24_abc-Elavl3-H2B-GC7f_gfapjRGECO1b"),
                                              ...
                                              shape=(subsampled_num_planes, downsampled_num_pixels * 4, downsampled_num_pixels * 2),
                                              location='center',
                                              strategy='center',
                                              bounds_roi='data',
                                              steps=(1, 1, 1),
...
def test():
    ...
    micdeconv = initialize_microscope_reconstruction(latest=latest)
    print(micdeconv)
    num_params = torch.cat([p.view(-1) for p in micdeconv['deconvs'].parameters()]).shape[0]
    print(num_params)
